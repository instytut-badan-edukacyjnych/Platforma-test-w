 ...
 */
public class CountingSummaryTask extends LookTask implements CountList.OnItemSelectedListener {
    ...
    private static final String ATTR_IS_LAST = "isLast";
    boolean showCorrect;
    ...
    private HashMap<CountList, Boolean> selectedBoards;

    private CountingTaskGroup.SummaryTaskResults countingTaskResultes;
    private boolean isLastInSeries = false;
                ...
                if (list.getLength() > a_info.m_groupIndex) {
                    ...
                    if (map != null) {
                        try {
                            ...
                            isLastInSeries = getString(map, ATTR_IS_LAST, "false").equals("true");
    ...
    public void setData(CountingTaskGroup.SummaryTaskResults countingTaskResultses) {
        ...
        if (view != null) {
            ...
            for (int i = 0; i < countingTaskResultses.resultses.size(); i++) {
                ...
                countList.setOnItemSelectedListener(this);
                selectedBoards.put(countList, false);
                container.addView(countList);
            }
        }
        button.setOnClickListener(v -> {
            for (int i = 0; i < boards.size(); i++) {
                boards.get(i).showCorrect(countingTaskResultes.resultses.get(i).correctBallsCount);
    ...
    public void setView(LinearLayout view) {
        ...
        continueButton.setOnClickListener(v -> {
            ((TaskAct) getContext()).getTaskHandler().sendEmptyMessageDelayed(BaseTask.TASK_MESS_NEXT_TASK, 10);
    ...
    @Override
    public void onItemSelected(CountList countList, int number) {
        if (number != -1) {
            selectedBoards.put(countList, true);
        }
        boolean allBoardAreSelected = true;
        for (Boolean aBoolean : selectedBoards.values()) {
            if (!aBoolean) {
                allBoardAreSelected = false;
                break;
            }
        }

        if (showCorrect && allBoardAreSelected) {
            button.setVisibility(View.VISIBLE);
            continueButton.setVisibility(View.GONE);
        } else if (allBoardAreSelected) {
            if (!isLastInSeries) {
                continueButton.setVisibility(View.VISIBLE);
            }
            button.setVisibility(View.GONE);
