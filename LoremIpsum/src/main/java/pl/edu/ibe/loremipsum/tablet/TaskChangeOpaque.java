...
/**
 * Klasa przełaczania zadań przez zmianę przeźroczystosci
 *
 *
 * @deprecated It may be used by reflection!
 */
@Deprecated
public final class TaskChangeOpaque extends TaskChangeBase {
    private static final String TAG = "TaskChangeOpaque";
    ...
    public void Change(TaskAct a_parent, final TaskView a_fore, final TaskView a_back) {
        ...
        final ObjectAnimator foreToback = ObjectAnimator.ofFloat(a_fore, "alpha", 1.0f, 0.0f);
        foreToback.setDuration(500);
        foreToback.setInterpolator(m_accelerator);
        final ObjectAnimator backTofore = ObjectAnimator.ofFloat(a_back, "alpha", 0.1f, 1.0f);
        backTofore.setDuration(800);
        backTofore.setInterpolator(m_decelerator);

        m_backView.setAlpha(0.1f);
        ...
        backTofore.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator anim) {
                try {
                    ...
                    LogUtils.e(TAG, "Exception durin onAnimationEnd", e);
                }
            }
        });
