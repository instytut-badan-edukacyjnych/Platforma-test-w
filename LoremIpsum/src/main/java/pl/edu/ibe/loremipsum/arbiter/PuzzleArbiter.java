...
/**
 * Marks task based on items location on specific fields
 *
 *
 */
public class PuzzleArbiter extends BaseArbiter {
//	private static final String TAG = "PuzzleArbiter";
    ...
    public void Assess(ArrayList<BaseTask.FieldSelect> a_data, String a_answer) {
        ...
        for (BaseTask.FieldSelect f : a_data) {
            ...
            if (a_answer.contains(f.m_name)) {
                if ((((f.m_srce.left + f.m_srce.right) / 2) != f.m_xPlace) ||
                        (((f.m_srce.top + f.m_srce.bottom) / 2) != f.m_yPlace)) {
                    passed = false;
                }
            }
        }
